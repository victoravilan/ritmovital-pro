    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4 relative">
      ...
      <Card className="w-full max-w-md bg-slate-800/50 border-slate-700">
        ...
        <CardContent className="space-y-4">
          ...
          {step === 2 && <><div><Label htmlFor="birthDate" className="text-white font-semibold">{t('fields.birth_date')}</Label><Input id="birthDate" type="date" value={profile.birthDate || ""} onChange={handleInputChange} className="bg-slate-700 border-slate-600 text-white font-semibold [color-scheme:dark]" /></div><div><Label htmlFor="birthTime" className="text-white font-semibold">{t('fields.birth_time')}</Label><Input id="birthTime" type="time" value={profile.birthTime || ""} onChange={handleInputChange} className="bg-slate-700 border-slate-600 text-white font-semibold [color-scheme:dark]" /></div></>}
